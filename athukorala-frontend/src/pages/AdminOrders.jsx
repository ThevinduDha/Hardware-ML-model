  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-8 md:p-20 font-sans selection:bg-[#D4AF37]">
      <header className="mb-16 flex justify-between items-end">
        ...
        <div className="hidden lg:flex gap-12 bg-white/[0.02] border border-white/5 p-8 backdrop-blur-md">
        ...
        <AnimatePresence>
          {orders.filter(o => filter === 'ALL' || o.status === filter).map((order) => (
            <motion.div 
              ...
              className="bg-white/[0.01] border border-white/5 p-6 flex flex-wrap lg:flex-nowrap items-center justify-between gap-8 hover:bg-white/[0.02] transition-colors"
            >
              <div className="flex items-center gap-6">
                <div className="p-4 bg-[#D4AF37]/10 text-[#D4AF37] rounded-full">
                  <Package size={20} />
                </div>
                <div>
                  <h3 className="font-mono font-bold text-lg tracking-widest">ATH-{order.id + 1000}</h3>
              ...
              <div className="flex-1 px-10">
                 <p className="text-[9px] text-gray-600 font-bold uppercase mb-1 tracking-widest">Destination</p>
                 <p className="text-xs uppercase text-gray-300 truncate max-w-sm">{order.shippingAddress}</p>
              </div>

              <div className="text-right mr-10">
                 <p className="text-[9px] text-[#D4AF37] font-bold uppercase mb-1 tracking-widest">Authorized Total</p>
                 <p className="text-xl font-black">LKR {order.totalAmount.toLocaleString()}</p>
              </div>
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className="p-3 bg-blue-500/10 text-blue-500 border border-blue-500/20 hover:bg-blue-500 hover:text-white transition-all rounded-sm title='Mark as Dispatched'"
                >
                  <Truck size={16} />
                </button>
                <button 
                  ...
                  className="p-3 bg-green-500/10 text-green-500 border border-green-500/20 hover:bg-green-500 hover:text-white transition-all rounded-sm title='Mark as Completed'"
                >
                  <CheckCircle size={16} />
                </button>
              </div>
